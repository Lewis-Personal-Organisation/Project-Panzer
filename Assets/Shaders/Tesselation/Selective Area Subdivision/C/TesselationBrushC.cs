...
public class TesselationBrushC : MonoBehaviour
{
    public SelectiveTessellatorC tessellator;
    ...
    [FoldoutGroup("Brush Settings")] [Min(0f)] public float flattenSpeed = 1f; // ADDED
    
    private float lastBrushTessellationRadius;
    private float lastBrushDeformRadius;
    private float lastBrushDepth;
    private float lastFlattenSpeed; // ADDED
    ...
    private void Update()
    {
        ...
        else if (!Mathf.Approximately(lastFlattenSpeed, flattenSpeed)) // ADDED
        {
            lastFlattenSpeed = flattenSpeed;
            rebuildTessellation = true;
        }
