...
public class SelectiveTessellatorB : MonoBehaviour
{
    struct VertexData
    {
        public Vector3 pos;
    ...
    [FoldoutGroup("Brush Settings")] public TesselationBrushB brush;
    ...
    private void InputDataChanged()
    {
        ...
        // float visualRadiusPadding = brush.includeIndicatorWidthInEffectRadius ? brush.brushIndicatorWidth * 0.5f : 0f;
        tessRadius = brush.brushTessellationRadius;// + Mathf.Max(0f, visualRadiusPadding);
    ...
    void RebuildTessellation()
    {
        ...
        for (int pass = 0; pass < passCount; pass++)
        {
            ...
            if (isFirstPass)
            {
                // Apply brush once to the persistent base vertices.
                ...
                tessShader.SetFloat("_FlattenSpeed", brush.flattenSpeed); // ADDED
                ...
                // If transform reference option is chosen, but not assigned a transform, use MaxDrop mode until it is
            ...
            {
                // Additional tessellation passes operate on pass-local vertices,
                // so persistent deformation from pass 1 is never overwritten/reset.
            ...
            int compactTriCount = RunSelectiveTessellation(triCount, usePersistentMask: isFirstPass && persistentTessellation, inputPositions);
    ...
    int RunSelectiveTessellation(int triCount, bool usePersistentMask, ComputeBuffer inputPositions)
    {
        ...
        tessShader.SetBuffer(kTess, "_OriginalVertices", originalPositions);
