...
public class LobbySetupUI : Panel
{
	...
	private void Awake()
	{
		...
		privateButton.onClick.AddListener(delegate
		{
			isLobbyPrivate = true;
			SetLobbyNameText();
			OnLobbyAccessTypeButtonClicked(new(.2F, privateButton.image, unselectedColour, selectedColour),
		...
		publicButton.onClick.AddListener(delegate
		{
			...
			SetLobbyNameText();
			OnLobbyAccessTypeButtonClicked(new(.2F, publicButton.image, unselectedColour, selectedColour),
	...
	private void OnLobbyAccessTypeButtonClicked(params Utils.LerpGroup[] lerpGroups)
	{
