...
public class LobbySetupUI : Panel
{
	...
	private void Awake()
	{
		...
		{ 
			...
			buttonColourOnCoroutine = StartCoroutine(Utils.ColourLerp(.2F, privateButton.image, selectableButtonFromColour, selectableButtonToColour));

			if (publicButton.image.color != selectableButtonFromColour)
				buttonColourOffCoroutine = StartCoroutine(Utils.ColourLerp(.2F, publicButton.image, selectableButtonToColour, selectableButtonFromColour));
			isLobbyPrivate = true;
		});
		...
		publicButton.onClick.AddListener(delegate 
		{
			...
			buttonColourOnCoroutine = StartCoroutine(Utils.ColourLerp(.2F, publicButton.image, selectableButtonFromColour, selectableButtonToColour));

			if (privateButton.image.color != selectableButtonFromColour)
				buttonColourOffCoroutine = StartCoroutine(Utils.ColourLerp(.2F, privateButton.image, selectableButtonToColour, selectableButtonFromColour));
