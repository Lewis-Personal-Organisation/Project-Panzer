...
public class LobbySetupUI : Panel
{
	...
	private void Awake()
	{
		...
		publicButton.onClick.AddListener(delegate 
		{
			...
			buttonColourOnCoroutine = StartCoroutine(Utils.ColourLerpOverTime(1, publicButton.image, selectableButtonFromColour, selectableButtonToColour));

			if (privateButton.image.color != selectableButtonFromColour)
				buttonColourOffCoroutine = StartCoroutine(Utils.ColourLerpOverTime(1, privateButton.image, selectableButtonToColour, selectableButtonFromColour));
