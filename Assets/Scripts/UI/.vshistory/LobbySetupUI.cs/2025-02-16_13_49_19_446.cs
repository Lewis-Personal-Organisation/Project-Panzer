...
public class LobbySetupUI : Panel
{
	...
	private void Awake()
	{
		...
		privateButton.onClick.AddListener(delegate
		{
			isLobbyPrivate = true;
			SetLobbyNameText();
			OnLobbyAccesTypeButtonClicks(new(.2F, privateButton.image, unselectedColour, selectedColour),
		...
		publicButton.onClick.AddListener(delegate
		{
			...
			SetLobbyNameText();
			OnLobbyAccesTypeButtonClicks(new(.2F, publicButton.image, unselectedColour, selectedColour),
	...
	private void OnLobbyAccesTypeButtonClicks(params Utils.LerpGroup[] lerpGroups)
	{
