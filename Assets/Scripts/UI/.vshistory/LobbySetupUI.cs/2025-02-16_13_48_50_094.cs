...
public class LobbySetupUI : Panel
{
	...
	private void Awake()
	{
		...
		privateButton.onClick.AddListener(delegate
		{
			isLobbyPrivate = true;
			SetLobbyNameText();
			OnButtonClick(new(.2F, privateButton.image, unselectedColour, selectedColour),
		...
		publicButton.onClick.AddListener(delegate
		{
			...
			SetLobbyNameText();
			OnButtonClick(new(.2F, publicButton.image, unselectedColour, selectedColour),
	...
	private void OnButtonClick(params Utils.LerpGroup[] lerpGroups)
	{
