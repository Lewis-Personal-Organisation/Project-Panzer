...
public class CloudSaveManager : Singleton<CloudSaveManager>
{
    ...
    public async Task LoadAndCacheData()
    {
        try
        {
            ...
            if (savedData.TryGetValue(k_PlayerStatsKey, out Item playerStatsJson))
            {
                m_PlayerStats = JsonUtility.FromJson<DataStructs.PlayerStats>(playerStatsJson.Value.GetAsString());
            }
            else
            {
