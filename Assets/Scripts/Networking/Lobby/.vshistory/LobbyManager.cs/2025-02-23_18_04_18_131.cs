...
public class LobbyManager : Singleton<LobbyManager>
{
	...
	public void OnLobbyChangedNotif(ILobbyChanges changes)
	{
		...
		else
		{
			...
			{
				Debug.Log($"LobbyManager :: {changes.PlayerData.Changed}, {changes.PlayerJoined.Changed}, {changes.PlayerLeft.Changed}");

				CacheLocalPlayer();

				if (activeLobby.Players.Exists(player => player.Id == playerId))
				{
					Debug.Log($"LobbyManager :: We exist. Checking if Game is ready'd up. Also adjusting player slots etc");
